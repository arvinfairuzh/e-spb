...
<form method="POST" autocomplete="off" action="<?= base_url('master/Permintaan_barang/insert_subkon') ?>" id="upload-create">
    ...
    <div class="form-group">
        ...
        <select style="width: 100%;" id="kode_cost" name="kode_pekerjaan[]" class="form-control select2">
            ...
            foreach ($cost_code as $cost_code_record) {
                echo "<option value=" . $cost_code_record['kode_costcontrol'] . ">" . $cost_code_record['kode_costcontrol'] . "</option>";
            }
            ?>
        </select>
    </div>
    <div class="form-group autocomplete">
        ...
        <input type="text" class="form-control" id="myInput" name="id_barang[]">
    </div>
    <div class="form-group">
        <label>Volume</label>
        <input style="" type="number" step="0.01" class="form-control qty" id="qty" name="qty[]">
    </div>
    <div class="form-group">
        <label>Satuan</label>
        <select style="width: 100%;" id="satuan" name="id_satuan[]" class="form-control satuan">
            ...
            foreach ($cost_code as $cost_code_record) {
                echo "<option value=" . $cost_code_record['uom'] . ">" . $cost_code_record['uom'] . "</option>";
            }
            ?>
        </select>
    </div>
    <div class="form-group">
        ...
        <input type="date" class="form-control" name="waktu_pelaksanaan[]" id="">
    </div>
    <div class="form-group">
        ...
        foreach ($master_persyaratan as $master_persyaratan_record) {
            echo "<input type='checkbox' value='" . $master_persyaratan_record['id'] . "' name='persyaratan[]'> " . $master_persyaratan_record['nama'] . '<br>';
        }
        ?>
    </div>
    ...
    <div class="form-group">
        <hr>
        <button type="submit" class="btn btn-primary btn-send">Tambah item</button>
        ...
        $.ajax({
            ...
            success: function(response, textStatus, xhr) {
                ...
                if (str.indexOf("success") != -1) {
                    ...
                    $(".btn-send").addClass("disabled").html('Tambah item').attr('disabled', false);
                } else {
                    form.find(".show_error").hide().html(response).slideDown("fast");
                    $(".btn-send").removeClass("disabled").html('Tambah item').attr('disabled', false);
                }
            },
